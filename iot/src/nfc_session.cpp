// NFC session implementation: encapsulates PN532 setup, polling, provisioning APDUs, and admin commands.

#include "nfc_session.h"
#include "provisioning_phase.h"
#include <esp_system.h>
#include <esp_log.h>
#include <cstring>

namespace {
  PN532_HSU* hsu = nullptr;
  PN532* nfc = nullptr;
  bool samOk = false;
  // Force provisioning modes
  bool forceProvPersistent = false;   // toggled via 'f' (stays ON until toggled off)
  bool forceProvOneShot = false;      // armed via 'F' (auto-clears after one success)
  uint32_t startMillis = 0;           // used to ignore early serial noise
  uint8_t aid[] = {0xF0,0x01,0x02,0x03,0x04,0x05}; // HCE AID

  void printHex(const uint8_t* d, size_t n) {
    for (size_t i = 0; i < n; ++i) {
      Serial.printf("%02X", d[i]);
      if (i + 1 < n) Serial.print(' ');
    }
    Serial.println();
  }

  bool waitForCard(uint32_t timeoutMs = 15000, uint8_t stableReads = 2, uint16_t pollMs = 60) {
    const uint32_t start = millis();
    uint8_t stable = 0;
    uint32_t lastLog = 0;
    Serial.println("Waiting for card (adaptive poll)...");
    while (true) {
      bool present = nfc->inListPassiveTarget();
      if (present) {
        if (++stable >= stableReads) {
          Serial.println("Card detected (stable)");
          return true;
        }
      } else {
        stable = 0;
      }
      uint32_t now = millis();
      if (now - lastLog >= 2000) { Serial.print('.'); lastLog = now; }
      if (timeoutMs && now - start >= timeoutMs) {
        Serial.println("\nTimeout waiting for card");
        return false;
      }
      delay(pollMs);
    }
  }

  bool waitForRemoval(uint32_t timeoutMs = 8000, uint8_t stableAbsentReads = 3, uint16_t pollMs = 80) {
    const uint32_t start = millis();
    uint8_t stableAbs = 0;
    while (true) {
      bool present = nfc->inListPassiveTarget();
      if (!present) {
        if (++stableAbs >= stableAbsentReads) {
          Serial.println("Card removed (stable)\n");
          return true;
        }
      } else {
        stableAbs = 0;
      }
      if (timeoutMs && millis() - start >= timeoutMs) {
        Serial.println("Removal wait timeout");
        return false;
      }
      delay(pollMs);
    }
  }

  bool apduRetry(const uint8_t* apdu, uint8_t apduLen, uint8_t* resp, uint8_t* respLen, uint8_t attempts = 3, uint16_t backoff = 60) {
    for (uint8_t i = 1; i <= attempts; ++i) {
      uint8_t len = *respLen;
      bool ok = nfc->inDataExchange((uint8_t*)apdu, apduLen, resp, &len);
      if (ok) { *respLen = len; return true; }
      if (i < attempts) delay(backoff);
    }
    return false;
  }

  bool ensureSAM(uint8_t attempts = 5, uint16_t backoff = 150) {
    for (uint8_t i = 1; i <= attempts; ++i) {
      if (nfc->SAMConfig()) return true;
      delay(backoff);
    }
    return false;
  }
}

namespace NfcSession {
  void begin(HardwareSerial& uart, int rxPin, int txPin, uint32_t baud) {
    uart.begin(baud, SERIAL_8N1, rxPin, txPin);
    delay(100);
    hsu = new PN532_HSU(uart);
    nfc = new PN532(*hsu);
    Serial.println("\n=== PN532 HCE READER (modular) ===");
    nfc->begin();
    ProvisioningPhase::begin();
    esp_log_level_set("Preferences", ESP_LOG_NONE); // suppress noisy NVS NOT_FOUND
    startMillis = millis(); // mark startup for serial noise suppression
    uint32_t ver = nfc->getFirmwareVersion();
    if (!ver) {
      Serial.println("PN532 NOT FOUND!");
      return;
    }
    Serial.printf("PN532 Firmware %d.%d\n", (int)((ver >> 16) & 0xFF), (int)((ver >> 8) & 0xFF));
    Serial.print("Configuring SAM...");
    samOk = ensureSAM(5, 200);
    Serial.println(samOk ? "OK" : "FAILED (will still attempt to poll)");
    Serial.printf("[PhaseA] Provisioned=%s  Force(persist)=%s  OneShot=%s\n",
                  ProvisioningPhase::isProvisioned() ? "YES" : "NO",
                  forceProvPersistent ? "YES" : "NO",
                  forceProvOneShot ? "ARMED" : "NO");
    Serial.println("[PhaseA] Serial: p=print, f=toggle persistent force, F=one-shot force, r=reset phone, C=clear ALL, v=validate cert, x=dump pub, h=help");
  }

  static void handleSerial() {
    static uint32_t lastCmdMs = 0; // debounce identical fast repeats
    static char lastCmd = 0;

    // Ignore serial input during first 1500ms of boot to avoid garbage triggering destructive commands
    if (millis() - startMillis < 1500) {
      while (Serial.available()) Serial.read(); // flush any noise
      return;
    }

    while (Serial.available()) {
      char c = Serial.read();
      if (c < 32 || c > 126) continue; // ignore non-printable
      uint32_t now = millis();
      if (c == lastCmd && now - lastCmdMs < 250) continue; // debounce bursts
      lastCmd = c; lastCmdMs = now;

      if (c == 'p') {
        String kid; bool have = ProvisioningPhase::getKeyId(kid);
        uint8_t pub[65]; size_t pubLen = ProvisioningPhase::getPhonePubRaw(pub, sizeof(pub));
        size_t certLen = ProvisioningPhase::getCertChain(nullptr, 0);
        Serial.println("[Admin] Stored provisioning state:");
        Serial.printf("  keyId: %s\n", have ? kid.c_str() : "<none>");
        Serial.printf("  pubKey: %s (len=%u)\n", (pubLen == 65 && pub[0] == 0x04) ? "present" : "<none>", (unsigned)pubLen);
        Serial.printf("  certLen: %u\n", (unsigned)certLen);
      } else if (c == 'f') {
        forceProvPersistent = !forceProvPersistent;
        Serial.printf("[Admin] Persistent force provisioning: %s\n", forceProvPersistent ? "ON" : "OFF");
      } else if (c == 'F') {
        forceProvOneShot = true;
        Serial.println("[Admin] One-shot force provisioning ARMED (will auto-clear after success)");
      } else if (c == 'r') {
        ProvisioningPhase::clearProvisionedOnly();
        Serial.println("[Admin] Cleared phone provisioning (kept device keypair)");
      } else if (c == 'C') {
        ProvisioningPhase::clearAll();
        Serial.println("[Admin] CLEARED ALL");
      } else if (c == 'v') {
        bool ok = ProvisioningPhase::validateStoredCertMatchesStoredPub();
        Serial.printf("[Admin] Cert vs pub match: %s\n", ok ? "YES" : "NO/UNAVAILABLE");
      } else if (c == 'x') {
        uint8_t pub[65]; size_t pubLen = ProvisioningPhase::getPhonePubRaw(pub, sizeof(pub));
        if (pubLen == 65) { Serial.print("[Admin] pubKey65: "); printHex(pub, 65); }
        else Serial.println("[Admin] pubKey not stored");
      } else if (c == 'h') {
        Serial.println("Commands: p f F r C v x h");
      }
    }
  }

  static void performProvisioningIfNeeded(uint8_t sw1, uint8_t sw2) {
  bool alreadyProv = ProvisioningPhase::isProvisioned();
  bool shouldProvision = ((forceProvPersistent || forceProvOneShot) || !alreadyProv) && (sw1 == 0x90 && sw2 == 0x00);
  if (!shouldProvision) return;

    // Base GET_CHALLENGE (Lc=0, Le=0)
    Serial.println("[PhaseA] Step1: Base GET_CHALLENGE (Lc=0)");
    uint8_t baseApdu[] = {0x00,0xCA,0x00,0x00,0x00,0x00};
    uint8_t baseResp[255]; uint8_t baseLen = sizeof(baseResp);
    Serial.print("[PhaseA] OUT Base APDU: "); printHex(baseApdu, sizeof(baseApdu));
    if (!(apduRetry(baseApdu, sizeof(baseApdu), baseResp, &baseLen, 5, 150) && baseLen >= 2 && baseResp[baseLen-2] == 0x90 && baseResp[baseLen-1] == 0x00)) {
      Serial.println("[PhaseA] Base GET_CHALLENGE failed");
      return;
    }
    Serial.printf("[PhaseA] IN Base Resp len=%u SW=%02X%02X\n", baseLen, baseResp[baseLen-2], baseResp[baseLen-1]);
    int payloadLen = baseLen - 2; const uint8_t* p = baseResp; int idx = 0;
    if (payloadLen < 1 + 65 + 2) { Serial.println("[PhaseA] Base payload too short"); return; }
    uint8_t keyIdLen = p[idx++]; if (keyIdLen > payloadLen - idx) { Serial.println("[PhaseA] keyIdLen OOB"); return; }
    String keyId; for (uint8_t k = 0; k < keyIdLen; ++k) keyId += (char)p[idx+k]; idx += keyIdLen;
    if (idx + 65 + 2 > payloadLen) { Serial.println("[PhaseA] Layout error before pubKey"); return; }
    const uint8_t* pubKey65 = p + idx; idx += 65;
    uint16_t certLen = (uint16_t)(p[idx] << 8 | p[idx+1]); idx += 2;
    const uint8_t* cert = p + idx; if (idx + certLen > payloadLen) { Serial.println("[PhaseA] certLen OOB"); return; }
    bool havePub = pubKey65[0] == 0x04;
    Serial.printf("[PhaseA] Parsed Base: keyIdLen=%u keyId='%s' pubKeyFirst=%02X certLen=%u\n", keyIdLen, keyId.c_str(), pubKey65[0], certLen);
    if (havePub) {
      Serial.print("[PhaseA] pubKey65: "); printHex(pubKey65, 65);
    }
    if (certLen > 0) {
      Serial.printf("[PhaseA] cert (first up to 32 bytes): ");
      size_t show = certLen < 32 ? certLen : 32;
      printHex(cert, show);
    } else {
      Serial.println("[PhaseA] cert: <none>");
    }

    // Signature step (Lc=24 challenge)
    Serial.println("[PhaseA] Step2: Signature GET_CHALLENGE (Lc=24 challenge)");
    uint8_t vehicleId[8]; uint8_t nonce[16];
    for (int i = 0; i < 8; ++i) vehicleId[i] = (uint8_t)esp_random();
    for (int i = 0; i < 16; ++i) nonce[i] = (uint8_t)esp_random();
    uint8_t challenge[24]; memcpy(challenge, vehicleId, 8); memcpy(challenge + 8, nonce, 16);
    Serial.print("[PhaseA] vehicleId: "); printHex(vehicleId, 8);
    Serial.print("[PhaseA] nonce: "); printHex(nonce, 16);
    Serial.print("[PhaseA] challenge(24): "); printHex(challenge, 24);
    uint8_t sigApdu[5 + sizeof(challenge) + 1]; uint8_t sIdx = 0;
    sigApdu[sIdx++] = 0x00; sigApdu[sIdx++] = 0xCA; sigApdu[sIdx++] = 0x00; sigApdu[sIdx++] = 0x00;
    sigApdu[sIdx++] = sizeof(challenge); memcpy(sigApdu + sIdx, challenge, sizeof(challenge)); sIdx += sizeof(challenge); sigApdu[sIdx++] = 0x00;
    Serial.print("[PhaseA] OUT Sig APDU: "); printHex(sigApdu, sIdx);
    uint8_t sigResp[255]; uint8_t sigLenTotal = sizeof(sigResp);
    if (!(apduRetry(sigApdu, sIdx, sigResp, &sigLenTotal, 5, 150) && sigLenTotal >= 2 && sigResp[sigLenTotal-2] == 0x90 && sigResp[sigLenTotal-1] == 0x00)) {
      Serial.println("[PhaseA] Signature GET_CHALLENGE failed");
      return;
    }
    Serial.printf("[PhaseA] IN Sig Resp len=%u SW=%02X%02X\n", sigLenTotal, sigResp[sigLenTotal-2], sigResp[sigLenTotal-1]);
    int sigPayloadLen = sigLenTotal - 2; if (sigPayloadLen < 2) { Serial.println("[PhaseA] Signature payload too short"); return; }
    uint16_t sigLen = (uint16_t)(sigResp[0] << 8 | sigResp[1]);
    if (2 + sigLen > sigPayloadLen) { Serial.println("[PhaseA] sigLen OOB"); return; }
    const uint8_t* sigDer = sigResp + 2;
    Serial.printf("[PhaseA] sigLen=%u (DER first up to 32 bytes): "); size_t showSig = sigLen < 32 ? sigLen : 32; printHex(sigDer, showSig);
    bool sigOk = false;
    if (sigLen > 0 && havePub) sigOk = ProvisioningPhase::verifySignatureP256(pubKey65, challenge, sizeof(challenge), sigDer, sigLen);
    else if (sigLen == 0) sigOk = true; // fallback
    Serial.printf("[PhaseA] sigLen=%u verify=%s\n", sigLen, sigOk ? "OK" : "FAIL");
    if (!sigOk) { Serial.println("[PhaseA] Credentials NOT stored (signature failure)"); return; }
    // If already provisioned and same keyId, skip re-store unless forced
    String existing; bool haveExisting = ProvisioningPhase::getKeyId(existing);
    bool sameKey = haveExisting && existing == keyId;
    if (!alreadyProv || forceProvPersistent || forceProvOneShot) {
      if (!sameKey) {
        if (forceProvPersistent || forceProvOneShot) ProvisioningPhase::storeKeyIdAsciiForce(keyId.c_str());
        else ProvisioningPhase::storeKeyIdAsciiIfEmpty(keyId.c_str());
      } else {
        Serial.println("[PhaseA] Same keyId as stored; skipping keyId re-store");
      }
      if (havePub) ProvisioningPhase::storePhonePubRaw(pubKey65);
      if (certLen > 0) ProvisioningPhase::storeCertChain(cert, certLen);
      Serial.println("[PhaseA] Provisioning data persisted (keyId/pub/cert as available)");
      if (forceProvOneShot) {
        forceProvOneShot = false; // auto-reset
        Serial.println("[PhaseA] One-shot force consumed; returning to normal mode");
      }
    } else {
      Serial.println("[PhaseA] Already provisioned; skipping store (toggle 'f' to force)");
    }
  }

  void tick() {
    handleSerial();
    if (!waitForCard(15000, 2, 60)) {
      if (!samOk) {
        Serial.println("Re-attempting SAMConfig...");
        samOk = ensureSAM(3, 200);
        if (samOk) Serial.println("SAM configured successfully");
      }
      return;
    }

    // SELECT AID
    uint8_t sel[16]; uint8_t idx = 0;
    sel[idx++] = 0x00; sel[idx++] = 0xA4; sel[idx++] = 0x04; sel[idx++] = 0x00;
    sel[idx++] = sizeof(aid); for (size_t i = 0; i < sizeof(aid); ++i) sel[idx++] = aid[i];
    sel[idx++] = 0x00; // Le
    uint8_t resp[32]; uint8_t rlen = sizeof(resp);
    if (!apduRetry(sel, idx, resp, &rlen, 3, 80)) {
      Serial.println("SELECT AID failed");
      waitForRemoval();
      return;
    }
    uint8_t sw1 = (rlen >= 2) ? resp[rlen - 2] : 0x00;
    uint8_t sw2 = (rlen >= 2) ? resp[rlen - 1] : 0x00;
    int payloadLen = (rlen >= 2) ? (rlen - 2) : 0;
  Serial.printf("SELECT SW1SW2=%02X%02X payloadLen=%d  Provisioned=%s  Force(persist)=%s  OneShot=%s\n",
          sw1, sw2, payloadLen,
          ProvisioningPhase::isProvisioned()?"YES":"NO",
          forceProvPersistent?"YES":"NO",
          forceProvOneShot?"ARMED":"NO");

    performProvisioningIfNeeded(sw1, sw2);

    waitForRemoval();
  }
}
